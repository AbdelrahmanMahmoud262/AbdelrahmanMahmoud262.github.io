...
export const metadata: Metadata = createMetadata({
  ...
  keywords: [
    ...
    "app blocker Android Kotlin",
    "privacy-first Android app",
    "UsageStats API Android",
  ],
  path: "/case-studies/rafiqy",
  ogImage: "/og/case-study-rafiqy.png",
});

export default function RafiqyCaseStudy() {
  const project = CASE_STUDIES.find((c) => c.id === "rafiqy")!;

  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32 flex flex-col gap-12 animate-fadeIn">
      <JsonLd data={softwareApplicationSchema({ name: project.title, description: project.tagline, url: `${tokens.site.url}/case-studies/rafiqy/` })} />
      <JsonLd data={breadcrumbSchema([{ name: "Home", url: `${tokens.site.url}/` }, { name: "Case Studies", url: `${tokens.site.url}/case-studies/` }, { name: project.title, url: `${tokens.site.url}/case-studies/rafiqy/` }])} />
      {/* Back button */}
      <div className="flex flex-col gap-4">
        ...
        <Breadcrumbs items={[
          ...
          { name: project.title, url: `/case-studies/rafiqy/` }
        ]} />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 py-8 border-y border-zinc-800/80">
        ...
        <div className="bg-[#121214]/20 border border-zinc-800/80 rounded-md p-6">
          ...
          <span className="text-sm font-bold text-[#00e5ff]">Android Accessibility API</span>
        </div>
        <div className="bg-[#121214]/20 border border-zinc-800/80 rounded-md p-6">
          <span className="block text-[10px] font-mono text-zinc-500 uppercase select-none">Architecture</span>
          <span className="text-sm font-bold text-white">Multi-Module (:feature:protection)</span>
        </div>
        <div className="bg-[#121214]/20 border border-zinc-800/80 rounded-md p-6">
          <span className="block text-[10px] font-mono text-zinc-500 uppercase select-none">Scheduler</span>
          <span className="text-sm font-bold text-white">UsageStats / WorkManager</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-10 text-sm text-zinc-300 leading-relaxed max-w-3xl">
        ...
        <div className="rounded-md border border-dashed border-zinc-800 bg-[#121214]/25 p-10 text-center flex flex-col gap-3 glass">
          ...
          <h4 className="text-sm font-bold text-white uppercase select-none">Accessibility & Foreground Services Lifecycle</h4>
          <p className="text-xs text-zinc-500 max-w-md mx-auto leading-relaxed">
            [System Diagram: How the UsageStats API registers app transitions to trigger the Accessibility Service, and how node inspectors process view bounds off-thread without creating UI jank.]
          </p>
        </div>
